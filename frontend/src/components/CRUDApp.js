...
const CRUDApp = ({ name, alias, datasource, pages, token, start_page_id, record_pk }) => {
  ...
  const [selectedPage, setSelectedPage] = useState(null); // nr of selected page
    ...
    function resolveDetailPages(detailPageRefs, allPages) {
      if (!detailPageRefs || detailPageRefs.length === 0) return null;
      return detailPageRefs.map(ref => {
        const found = allPages.find(p => p.alias === ref.alias);
        return found ? { ...ref, pageConfig: found } : null;
      }).filter(Boolean);
    }
