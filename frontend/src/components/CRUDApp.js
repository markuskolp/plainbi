              ...
              <Layout>
                ...
                <Sider width={250} theme={theme} collapsible  breakpoint="lg" collapsedWidth="0" onBreakpoint={broken => {console.log(broken);}} onCollapse={(collapsed, type) => {console.log(collapsed, type);}} >
                  ...
                  <Content style={{ background: "#FFF"}}>
                    ...
                    {page && 
                    <CRUDPage key={page.name} name={page.name} tableName={page.table} tableForList={page.table_for_list} tableColumns={page.table_columns} pkColumns={page.pk_columns ? page.pk_columns : null} userColumn={page.user_column ? page.user_column : null} allowedActions={page.allowed_actions} versioned={page.versioned ? page.versioned : false} datasource={datasource} isRepo={(datasource == "repo" || datasource == "0") ? "true" : "false"} lookups={getLookups(page.table_columns)} token={token} sequence={page.sequence ? page.sequence : null} breadcrumbItems={(page.show_breadcrumb && page.show_breadcrumb === 'true') ? [{title:page.parent_page.name, href:'/apps/'+alias+'/'+page.parent_page.alias}, {title:page.name}] : null} />
